if Meteor.isClient
  ...
  class @Calendar
    ...
    constructor: (element, options)->
      ...
      if !element
        console.error 'Need to provide target element to Aura::calendar'
      @el = element
      if options
        @group = options.group
        ...
        @drawGroups = options.drawGroups or @drawGroups
      ...
      if day < (startDay + 2) or day > (lastDay + startDay + 1)
        ...
        for num in [0..2]
          if @schedule[@month]
            ...
            console.error 'Aura::calendar: No schedule for this month:('
        markup += '</div></div>'
        markup
